...
_M.getallsitepolicyid = function(self)
    local runtimeInfoKey    = table.concat({self.baseLibrary, "*", fields.divDataKey}, separator)
    local database      = self.database

    local keys, err  = database:keys(runtimeInfoKey)
    if not keys or  keys == ngx.null then
        return false
    end
    local policyids = {}
    for _, v in pairs(keys) do
        local info,err= database:mget(v)
        policyids[#policyids+1] = info[1]
        info = {}
    end 
    if not policyids or  policyids == ngx.null then
        return false
    end
    return policyids
end

...
_M.checkpolicyid = function(self, id)
    local divTypeKey    = table.concat({self.baseLibrary, id, fields.divtype}, separator)
    local database      = self.database

    local divtype, err  = database:get(divTypeKey)
    if not divtype or  divtype == ngx.null then
        error{ERRORINFO.REDIS_ERROR, err}
    end
    return divtype
end
_M.update = function(self, policy)
    local id = policy.policyid
    local database = self.database
    local divModulename = table.concat({'abtesting', 'diversion', policy.divtype}, '.')
...
_M.ispolicyid = function(self, policyid,runtimeInfo)
    local divDataKey        = table.concat({self.baseLibrary, policyid, fields.divdata}, separator)
    for _, v in pairs(runtimeInfo) do
        if v == divDataKey then
            return v
        end
    end
